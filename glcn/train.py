...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'scence1', 'Dataset string.')  # 'cora', 'citeseer'
...
flags.DEFINE_float('losslr1', 0.0001, 'xxxx')#0.01
flags.DEFINE_float('losslr2', 0.0001, 'xxxx')
flags.DEFINE_integer('seed', 123, 'Number of epochs to train.')
flags.DEFINE_float('decay_lr', 1.0, 'Decay of the learning rate ')#0.9
flags.DEFINE_integer('pre', 1, 'Percentage of training set')

flags.DEFINE_float('lamb', 0.1, 'lamb.')
flags.DEFINE_float('lamb1', 0.01, 'lamb1.')
flags.DEFINE_string('wi', 'rnp15w5s30', 'wi.') # rnp15w530
...
# Create model
model = SGLCN(L,placeholders, edge, input_dim=features[2][1], logging=True)
...
print("----------------------------------------------")
print("The finall result:", test_acc)
print("The finall result:w_f1=", w_f1 )
print("The finall result:m_f1=", m_f1)
print("----------------------------------------------")

file_handle = open('scence750-sparse_glcnmr-renode.txt', mode='a')
file_handle.write(FLAGS.dataset+"\n"+"{:.9f}".format(FLAGS.lamb)+"\n"+"{:.9f}".format(FLAGS.lamb1)+"\n"
